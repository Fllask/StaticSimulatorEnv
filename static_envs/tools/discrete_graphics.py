...
def draw_grid(maxs,label_points=False,steps=1,color='darkslategrey',h=6,w=None,linewidth=1):
    ...
    return fig,ax
def draw_action(ax,rid,action,blocktype,grid,animated=False,multi=False,**action_args):
    action_args['rid']=rid
    if multi:
        x = action_args['x'][rid]
        y = action_args['y'][rid]
        ...
        y = action_args['y']
    if action in  ['Ph','P']:
        if 'x' in action_args.keys():
            art = draw_put(ax,blocktype,rid=rid,blocktypeid=None,hold=True,pos=[x,y],ori=None,animated=animated,multi=multi)
        else:
...
def draw_action_rel(ax,rid,action,blocktype,grid,animated=False,multi=False,**action_args):
    ...
    if action in  ['Ph','P']:
        ...
        if multi:
            grid.connect(blocktype, None,
                         action_args['sideblock'][rid],
                         action_args['sidesup'][rid],
                         action_args['bid_sup'][rid],
                         action_args['side_ori'][rid],
                         idcon=action_args['idconsup'][rid])
        else:
        ...
        assert False, f"action {action} is unknown"
    return art
def draw_put_rel(ax,
                 blocktype,
                 rid,
    ...
    return arts
def draw_put(ax,blocktype,rid,hold,pos,ori,blocktypeid,animated=False,multi=False,empty=True):
    if empty:
        arts = draw_block(ax, blocktype,pos, color='none',animated=animated)
        if hold:
            #the block was already moved by draw_block
            parts = blocktype.parts
            sides = np.concatenate([np.tile(parts,(3,1)),np.repeat(np.arange(3),parts.shape[0])[...,None]],axis=1)
...
def draw_leave(ax,rid,animated=False,multi=False):
    ...
    return [art]
def draw_stay(ax,rid,animated=False,multi=False,**useless_args):
    if multi:
    ...
    for r in rids:
        ...
        if len(bid_held>0):
            ...
            if uniform_scale is None:
                ...
                scale = uniform_scale
            if f2 > 1e-6:
                arts+=[ax.arrow(cm[0]+Mr*fr_y/f2, cm[1]-Mr*fr_x/f2,fr_x*scale,fr_y*scale,
                                            color=robot_colors[r],
                                            width=0.07,
                                            animated= animated,
                                            zorder=101)]
            if abs(Mr) > 1e-6 and f2<1e-6:
...
def fill_grid(ax,
              ...
              obstacle_color = 'xkcd:vermillion',
              graph = None,
              connectivity = None,
              forces_bag=None,
              draw_arrows = True,
    ...
    for i in ids:
        if i == -2:
            ...
            arts += fill_triangle(ax, coords.T,color=obstacle_color,animated=animated)
            continue
        if i == -1:
            continue
        if i ==0:
                ...
                if c is not None:
                    ...
                    if len(x_robot)>0:
                        ...
                        if len(forces)>0:
                            ...
                            minforce=1 #dont really care, this is mostly for debug
...
def draw_block(ax,block, pos=None,ori=None,draw_neigh=False,highlight_ref = False,animated=False, **colorkw):
    ...
    return arts
def draw_robot(ax,grid,robotid,base=None,animated=False,max_dist = None,right=True,dash=None,actuator_pos=None):
    if base is None:
        if max_dist is None:
            if robotid == 0:
...
def save_anim(ani,name='animation',ext = 'html',fps=3):
    if ext == 'html':
...
if __name__ == "__main__":
    ...
    fig,ax = draw_grid(maxs,steps=1,color='k',label_points=False,h=6,linewidth=0.3)
    fill_grid(ax,grid)#,fixed_color=[0.5,0.5,0.5])
